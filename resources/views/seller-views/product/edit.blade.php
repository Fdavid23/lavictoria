    ...
    <div class="row">
        @foreach (json_decode($product->images) as $key => $photo)
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        <img style="width: 100%" src="{{asset("storage/app/public/product/$photo")}}" alt="Product image">
                        <a href="{{route('seller.product.remove-image',['id'=>$product['id'],'name'=>$photo])}}"
                           class="btn btn-danger btn-block" style="border-radius: 0;margin-top: -30px">Remove</a>
                    </div>
                </div>
            </div>
        @endforeach
